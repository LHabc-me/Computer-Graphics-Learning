...
static bool insideTriangle(int x, int y, const Vector3f* _v) {
    ...
    // V0P*V0V1、V1P*V1V2、V2P*V2V0都>=0，P在三角形内
    Eigen::Vector2f p;
    p << x, y;
...
void rst::rasterizer::rasterize_triangle(const Triangle& t) {
    ...
    for (int x = min_x; x < max_x; ++x) {
        for (int y = min_y; y < max_y; ++y) {
            int cnt = 0;
            float z = MAXFLOAT;
            for (auto const& p: pix) {
                if (insideTriangle(x + p[0], y + p[1], t.v)) {
                    ++cnt;
                    auto [alpha, beta, gamma] = computeBarycentric2D(x + p[0], y + p[1], t.v);
